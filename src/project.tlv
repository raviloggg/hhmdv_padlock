...
module smart_padlock(
    input clk,                // Clock signal
    input reset,              // Reset signal
    input [3:0] keypad,       // 4-bit input from keypad
    input enter,              // Signal to submit a digit
    output reg unlock,        // Unlock signal
    output reg error          // Error signal
);

    // Parameters
    parameter PIN_LENGTH = 4;
    reg [3:0] stored_pin [0:PIN_LENGTH-1];    // Predefined PIN
    reg [3:0] entered_pin [0:PIN_LENGTH-1];   // User-entered PIN
    reg [1:0] index;                          // Index for entered PIN

    // State Machine
    typedef enum {IDLE, INPUT, CHECK, DONE} state_t;
    state_t current_state, next_state;

    // Initialize stored PIN
    initial begin
        stored_pin[0] = 4'd1;  // PIN = 1234
        stored_pin[1] = 4'd2;
        stored_pin[2] = 4'd3;
        stored_pin[3] = 4'd4;
    end
    ...
    always @(*) begin
        ...
        case (current_state)
            ...
            INPUT: begin
                if (index < PIN_LENGTH) begin
                    entered_pin[index] = keypad;  // Capture input digit
                    if (enter) begin
            ...
            CHECK: begin
                if (entered_pin == stored_pin) begin
                    unlock = 1;  // Unlock if PIN matches
                end else begin
                    error = 1;   // Error if PIN mismatch
                end
                next_state = DONE;
            end
